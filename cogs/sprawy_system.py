...
# Kanały i kategorie
TICKET_CHANNEL_ID = 1369721802108178563  # ID kanału z formularzem
TICKET_CATEGORY_ID = 1368264010767667343  # ID kategorii dla ticketów

# Konfiguracja typów ticketów
TICKET_TYPES = {
	"sadstanowy": {
		"label": "Dokumenty do Sądu Stanowego",
		"handler_roles": [1334888295154319522],  # pingowane
		"viewer_roles": [1334892405035372564], # tylko dostęp
		"fields": [
			("Imię i nazwisko (IC)", "np. John Doe", True),
			("Stanowisko i Wydział", "np. Judge, Sądownictwo", True),
			("Numer Sprawy", "JEŚLI DOTYCZY", False),
			("Opisz Sprawę", "Napisz co potrzebuje/chcesz złożyć itp.", True),
			("Inne uwagi/Załączniki", "POLE NIEOBOWIĄSKOWE", False)
		]
	},
	"sadfederalny": {
		"label": "Dokumenty do Sądu Federalnego",
		"handler_roles": [1359160145765990420],  # pingowane
	...
	"sadapelacyjny": {
		"label": "Dokumenty do Sądu Apelacyjnego",
		"handler_roles": [1347928139900256286],  # pingowane
		"viewer_roles": [1359160145765990420], # tylko dostęp
		"fields": [
			("Imię i nazwisko (IC)", "np. John Doe", True),
			("Stanowisko i Wydział", "np. Judge, Sądownictwo", True),
			("Numer Sprawy", "JEŚLI DOTYCZY", False),
	...
	"prokuratura": {
		"label": "Dokumenty do Prokuratury",
		"handler_roles": [1359162624889065512],  # pingowane
		"viewer_roles": [1334891241002893312], # tylko dostęp
		"fields": [
			("Imię i nazwisko (IC)", "np. John Doe", True),
			("Stanowisko i Wydział", "np. Judge, Sądownictwo", True),
			("Numer Sprawy", "JEŚLI DOTYCZY", False),
	...
	"adwokatura": {
		"label": "Dokumenty do Adwokatury",
		"handler_roles": [],  # pingowane
		"viewer_roles": [1334881150925275187], # tylko dostęp
		"fields": [
			("Imię i nazwisko (IC)", "np. John Doe", True),
			("Stanowisko i Wydział", "np. Judge, Sądownictwo", True),
			("Numer Sprawy", "JEŚLI DOTYCZY", False),
	...
	"irs": {
		"label": "Dokumenty do IRS",
		"handler_roles": [1334881150858035299, 1334881150895652885],  # pingowane
		"viewer_roles": [1334881150858035297], # tylko dostęp
		"fields": [
			("Imię i nazwisko (IC)", "np. John Doe", True),
			("Stanowisko i Wydział", "np. Judge, Sądownictwo", True),
			("Numer Sprawy", "JEŚLI DOTYCZY", False),
	...
	"oig": {
		"label": "Dokumenty do Biuro Ins. Generalnego",
		"handler_roles": [1334881150895652891],  # pingowane
		"viewer_roles": [1334881150895652888], # tylko dostęp
		"fields": [
			("Imię i nazwisko (IC)", "np. John Doe", True),
			("Stanowisko i Wydział", "np. Judge, Sądownictwo", True),
			("Numer Sprawy", "JEŚLI DOTYCZY", False),
...
class SprawySystem(commands.Cog):
	...
	async def on_ready(self):
		print("SprawySystem cog załadowany.")
